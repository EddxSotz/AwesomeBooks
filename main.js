...
//Remove book function takes 'book' parameter from 'displayAllBooks' function and removes only that element and update local Storage
  removeBook(book) {
    bookList = bookList.filter((element) => element !== book);
    localStorage.setItem('BooksList', JSON.stringify(bookList));
    this.displayAllBooks();
  }
//Clears any previous book list and loops through each book element creating the elements and calling the 'removeBook' function when user clicks the remove btn
  displayAllBooks() {
    allBooks.innerHTML = '';
...
// receive the parameters and create an object from them to push it to the global 'bookList' variable and update local Storage then display all books
...
//Listen when form is busmitted and call addBook function with parameters then display all books if not empty and show error message if it is
